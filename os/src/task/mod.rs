...
impl TaskManager {
    ...
    fn get_current_status(&self) -> TaskStatus {
        let inner = self.inner.exclusive_access();
        inner.tasks[inner.current_task].task_info.status
    }
    ...
    fn get_current_total_time(&self) -> usize {
        let inner = self.inner.exclusive_access();
...
/// Get the current task's total time
pub fn get_current_total_time() -> usize {
    TASK_MANAGER.get_current_total_time()
}
